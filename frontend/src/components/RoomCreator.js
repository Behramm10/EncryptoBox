...
const RoomCreator = ({ onRoomCreated }) => {
  ...
  const handleJoinRoom = async (e) => {
    ...
    try {
      ...
      onRoomCreated({ roomId: response.room.id, ttl: response.room.ttl, expiresAt: response.room.expiresAt, isNewRoom: false });
    } catch (err) {
      setError(err.message);
    } finally {
      setIsJoining(false);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-12">
        ...
        <h2 className="text-4xl font-bold neon-text mb-4">Welcome to EncryptoBox</h2>
        <p className="text-lg text-gray-600 dark:text-gray-400 mb-2 max-w-2xl mx-auto">Create a secure, self-destructing chat room or join an existing one.</p>
        <p className="text-sm text-gray-500 dark:text-gray-500">All messages are encrypted on your device and automatically deleted after TTL expires.</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="card p-8 glow-effect hover:scale-[1.02] transition-all duration-300">
          <div className="text-center mb-6">
            ...
            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-2">Join Existing Room</h3>
            <p className="text-gray-600 dark:text-gray-400 text-sm">Enter a room ID to join an existing conversation.</p>
          </div>
          <form onSubmit={handleJoinRoom}>
            <div className="mb-6">
              <input type="text" id="roomId" value={roomId} onChange={(e) => setRoomId(e.target.value)} placeholder="Enter room ID..." className="input-field" required />
            </div>
            <button type="submit" disabled={isJoining || !roomId.trim()} className="w-full btn-primary disabled:opacity-50 disabled:cursor-not-allowed">
              {isJoining ? (
                <span className="flex items-center justify-center">
                  <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
        ...
        <div className="flex items-start gap-4">
          <div className="flex-shrink-0">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-500/20 to-accent-500/20 border border-primary-400/30 flex items-center justify-center">
              <svg className="h-5 w-5 text-primary-400" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" /></svg>
            </div>
          </div>
          <div>
            <h3 className="text-sm font-semibold text-primary-700 dark:text-primary-300 mb-3">Security Features</h3>
            <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
              <li className="flex items-start gap-2"><span className="text-primary-400 mt-0.5">✓</span><span>All messages encrypted with AES-256-GCM before leaving your device</span></li>
              <li className="flex items-start gap-2"><span className="text-primary-400 mt-0.5">✓</span><span>Messages automatically self-destruct after TTL expires</span></li>
              <li className="flex items-start gap-2"><span className="text-primary-400 mt-0.5">✓</span><span>Room passwords never sent to servers</span></li>
              <li className="flex items-start gap-2"><span className="text-primary-400 mt-0.5">✓</span><span>Zero-knowledge architecture - we cannot read your messages</span></li>
            </ul>
          </div>
        </div>
      </div>
