...
function App() {
    ...
    const handleInviteLink = async () => {
      ...
      if (roomId && inviteToken) {
        ...
        try {
          ...
          setCurrentRoom({ roomId: response.room.id, ttl: response.room.ttl, expiresAt: response.room.expiresAt, isNewRoom: false, inviteToken });
  ...
  const handleRoomCreated = (roomData) => setCurrentRoom(roomData);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-900 to-gray-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold neon-text mb-3">Connecting...</h2>
          <p className="text-gray-400 mb-6 max-w-md mx-auto">{connectionError || 'Establishing secure connection to server'}</p>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-900 to-gray-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold neon-text mb-3">Joining Room...</h2>
          <p className="text-gray-400">Opening secure room via invite link</p>
        </div>
      </div>
    );
  }
    ...
    <ToastProvider>
      <div className={`min-h-screen ${theme === 'dark' ? 'bg-gradient-to-br from-gray-900 via-gray-900 to-gray-950' : 'bg-gradient-to-br from-gray-50 via-white to-gray-100'}`}>
            ...
            <div className="flex justify-between items-center h-20">
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-3">
                  ...
                  <h1 className="text-2xl font-bold neon-text">EncryptoBox</h1>
                </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <button onClick={toggleTheme} className="p-2.5 rounded-xl glass hover:bg-white/10 transition-all duration-300 hover:scale-110" aria-label="Toggle theme">
                  {theme === 'dark' ? (
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {currentRoom ? (
            <ChatRoom roomData={currentRoom} onLeaveRoom={handleLeaveRoom} />
          ) : (
        ...
        <footer className="glass border-t border-white/10 mt-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="text-center space-y-2">
              <p className="text-sm text-gray-400">🔒 All encryption happens on your device. Messages self-destruct automatically.</p>
              <p className="text-xs text-gray-500">Built with React, Node.js, and Redis. No plaintext is ever stored on our servers.</p>
            </div>
          </div>
        </footer>
      </div>
